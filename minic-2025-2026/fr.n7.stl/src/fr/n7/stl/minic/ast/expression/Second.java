...
public class Second implements AccessibleExpression {
	...
	public Type getType() {
		...
		if (targetType instanceof fr.n7.stl.minic.ast.type.CoupleType) {
			return ((fr.n7.stl.minic.ast.type.CoupleType) targetType).getSecond();
		} else {
	...
	public Fragment getCode(TAMFactory _factory) {
		...
		int firstSize = ((fr.n7.stl.minic.ast.type.CoupleType)this.target.getType()).getFirst().length();
		int secondSize = this.getType().length();
