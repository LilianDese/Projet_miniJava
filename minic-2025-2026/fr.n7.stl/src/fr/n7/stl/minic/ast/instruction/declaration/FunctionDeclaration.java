...
public class FunctionDeclaration implements DeclarationInstruction {
	...
	public boolean collectAndPartialResolve(HierarchicalScope<Declaration> _scope) {
		...
		for (ParameterDeclaration param : this.parameters) {
			if (paramScope.accepts(param)) {
				paramScope.register(param);
			} else {
				Logger.error("Parameter " + param.getName() + " already declared.");
				ok = false;
			}
		}
	...
	public boolean completeResolve(HierarchicalScope<Declaration> _scope) {
		...
		boolean ok = true;
		for (ParameterDeclaration param : this.parameters) {
			ok &= param.getType().completeResolve(_scope);
		}

		ok &= this.type.completeResolve(_scope);

		ok &= this.body.completeResolve(paramScope);
